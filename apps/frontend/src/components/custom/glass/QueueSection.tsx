...
export const QueueSection = ({
  ...
  <div className="backdrop-blur-md bg-white/5 rounded-xl border border-white/20 shadow-lg">
    <div className="p-4 flex flex-col">
      ...
      {queue.length === 0 ? (
        ...
        <div
          className="space-y-2 overflow-y-auto scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent pr-2"
          style={{ height: "calc(5 * (68px + 8px))" }}
        >
          {queue.map((queuedTrack, index) => (
            <div
              key={index}
              className="backdrop-blur-sm bg-white/5 rounded-lg p-3 border border-white/10 hover:bg-white/10 transition group"
            >
              <div className="flex justify-between items-center gap-2">
                ...
                {isOwner && (
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      onClick={() =>
                        onRemoveTrack && onRemoveTrack(queuedTrack)
                      }
